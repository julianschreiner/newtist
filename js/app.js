...
app.controller("myCtrl", function(
  ...
  angular.element($window).on("load", function() {
    if (
      ...
      parseInt($window.innerWidth) > 996
    ) {
      $scope.textLimit = 20;
      $scope.artistTextLimit = 20;
      $scope.$apply();
    } else {
  ...
  console.log("artist: ", $scope.artistTextLimit);
  ...
  $scope.getNewReleases = function(artistName = null, endless = false) {
    ...
    $.ajax({
      url:
        "https://api.spotify.com/v1/browse/new-releases?limit=" +
        $scope.searchReqLimit +
        "&offset=" +
        $scope.searchReqOffset,
      type: "GET",
      headers: {
        Authorization: "Bearer " + access_token
      },
  ...
  $scope.getNotification = function() {
    ...
    var json =
      '{"frouter": {"apikey": "1234", "method": "notificationHandle", "userID":  "' +
      $scope.userID +
      '" } }';
    obj = JSON.parse(json);
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        for (var i = 0; i < data["artists"]["items"].length; i++) {
          ...
          if (
            artistName.toUpperCase().includes($scope.inp_search.toUpperCase())
          ) {
  ...
  $scope.loadMore = function() {
    $scope.searchReqOffset += 50;
    console.log($scope.searchReqOffset);
    $scope.isLoading = true;
  ...
  $scope.subscribe = function(artistName) {
    ...
    var json =
      '{"frouter": {"apikey": "1234", "method": "subscribe", "artistName": "' +
      artistName +
      '", "userID": "' +
      $scope.userID +
      '"  } }';
    ...
    $http({
      ...
      function(response) {
        ...
        if (subSuccess === true) {
          ...
          $scope.isSubd = true;
          $scope.subMessage =
            "Successfully subscribed! You'll be informed when the artist releases something new.";
        }
      },
      function(response) {
        // failed
  ...
  $scope.unsubscribe = function(artistName) {
    ...
    var json =
      '{"frouter": {"apikey": "1234", "method": "unsubscribe", "artistName": "' +
      artistName +
      '", "userID": "' +
      $scope.userID +
      '"  } }';
  ...
  $scope.isSub = function(artistName) {
    var json =
      '{"frouter": {"apikey": "1234", "method": "isSub", "artistName": "' +
      artistName +
      '", "userID": "' +
      $scope.userID +
      '"  } }';
  ...
  $scope.favor = function(e, track, artist) {
    ...
    if (window.localStorage.getItem("favor") != null) {
      ...
      var arr = Object.keys(storageOBJ).map(function(k) {
        return storageOBJ[k];
      });
      arr.push(obj);
